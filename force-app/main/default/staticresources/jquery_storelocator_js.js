...
$.fn.storeLocator = function(options) {
  ...
  var settings = $.extend( {
      ...
      'slideMap'      : true,
      'modalWindow'   : false,
      'propertyFunction' : MapController.getProperties, // Apex RemoteAction
      'propertyFunctionParams' : false
  }, options);
  ...
  function mapping(orig_lat, orig_lng) {
      ...
      $(function() {
          // This function must follow the RemoteAction protocol:
          // [parameters...,]  - parameters to apex method
          // callbackFunction, - javascript callback (result, status)
          // [configuration]   - escape response? default is true
          settings.propertyFunction(
              // [parameters...,]  - parameters to apex method
              settings.propertyFunctionParams,
              ...
              $(function(){
                  ...
                  function create_store_variables(loopcount)
                  {
                    ...
                    storeLat = locationset[loopcount][2];
                    storeLng = locationset[loopcount][3]
                  ...
                  for (var y = 0; y <= storenum; y++)
                  {
                    ...
                    create_infowindow(marker, storeDistance, storeName, storeAddress1, storeAddress2, storeCity, storeState, storeZip, storePhone, storeWeb, storeHours1, storeHours2, storeHours3, 0, storeLat, storeLng);
                   }
                   ...
                   $(markers).each(function(x, marker){
                    ...
                    listClick(letter, marker, storeDistance, storeName, storeAddress1, storeAddress2, storeCity, storeState, storeZip, storePhone, storeWeb, storeHours1, storeHours2, storeHours3, storeLat, storeLng);

                  });

                  function listClick(letter, marker, distance, name, address1, address2, city, state, zip, phone, web, hours1, hours2, hours3, lat, lng)
                  {
                    $('<li />').html("<div class=\"list-label\">" + letter + "<\/div><div class=\"list-details\"><div class=\"list-content\"><div class=\"loc-name\">" + storeName + "<\/div> <div class=\"loc-addr\">" + storeAddress1 + "<\/div> <div class=\"loc-addr2\">" + storeAddress2 + "<\/div> <div class=\"loc-addr3\">" + storeCity + ", " + storeState + " " + storeZip + "<\/div> <div class=\"loc-phone\">" + storePhone + "<\/div> <div class=\"loc-web\"><a href=\"http://" + storeWeb + "\" target=\"_blank\">" + storeWeb + "</a><\/div><div class=\"loc-distance\">"+storeDistance.toFixed(2)+" miles<\/div><\/div><\/div>").click(function(){
                      map.panTo(marker.getPosition());
                      var listLoc = "left";
                      if(settings.bounceMarker == true)
                        {
                          marker.setAnimation(google.maps.Animation.BOUNCE)
                          setTimeout(function() { marker.setAnimation(null); create_infowindow(marker, name, address1, address2, city, state, zip, phone, web, hours1, hours2, hours3, listLoc, lat, lng); }, 700);
                        }
                        else
                        {
                          create_infowindow(marker, name, address1, address2, city, state, zip, phone, web, hours1, hours2, hours3, listLoc, lat, lng);
                        }
                  ...
                  function createMarker(point, name, address, letter) {
                    ...
                    var pinImage = new google.maps.MarkerImage("http://chart.apis.google.com/chart?chst=d_map_spin&chld=0.7|0|"+settings.pinColor+"|13|_|" + letter ,
                  ...
                  //Infowindows
                  function create_infowindow(marker, storeDistance, name, address1, address2, city, state, zip, phone, web, hours1, hours2, hours3, location, lat, lng)
                  {

                    var formattedAddress = "<div class=\"loc-name\">" + name + "<\/div>" + address_line(address1) + address_line(address2) + city + ", " + state + " " + zip + address_line(hours1) + address_line(hours2) + address_line(hours3) + address_line(phone) + "<div><a href=\"http://" + web + "\" target=\"_blank\">" + web + "<\/a><\/div><div>"+storeDistance.toFixed(2)+" miles</div><div class=\"lon-lat\">Lat-Lng: (" + lat+","+lng + ")<\/div>";
