...
BaseUIManager.instance = null;

BaseUIManager.prototype.construct = function () {
...
BaseUIManager.oldWindowOnload = window.onload;
window.onload = function () {
    $.isFunction(BaseUIManager.oldWindowOnload) ? BaseUIManager.oldWindowOnload() : null;
    BaseUIManager.instance.signalQueueReady('sfdcPageReady');
}
*/

BaseUIManager.prototype.attachToWindowOnload = function () {
    // IMPORTANT! attach to AFTER the current window.onload function, so that we may detect when the sfdcPage (if any) is ready -- actions like 
    // jController.driver* can cause problems for the sfdcPage if they are invoked before the sfdcPage's init sequences (e.g. dependent picklist init)
    if (!BaseUIManager.onloadHandlers || BaseUIManager.onloadHandlers.new !== window.onload) {
        BaseUIManager.onloadHandlers = {
            "old": window.onload,
            "new": function () {
                $.isFunction(BaseUIManager.onloadHandlers.old) ? BaseUIManager.onloadHandlers.old.call(document.body) : null;
                BaseUIManager.instance.signalQueueReady('sfdcPageReady');
            }
        }
        window.onload = BaseUIManager.onloadHandlers.new;
        /*
        BaseUIManager.newWindowOnload = window.onload;
        window.onload = function () {
            $.isFunction(BaseUIManager.oldWindowOnload) ? BaseUIManager.oldWindowOnload() : null;
            BaseUIManager.instance.signalQueueReady('sfdcPageReady');
        }
...
BaseUIManager.prototype.queue = function (queueName, func) {
...
BaseUIManager.prototype.comboQueue = function (queueNames, func) {
...
BaseUIManager.prototype.signalQueueReady = function (queueName) {
...
BaseUIManager.prototype.queueOnSfdcPageReady = function (func) {
    this.queue('sfdcPageReady', func);
}

BaseUIManager.prototype.queueOnCompleteAction = function (func) {
    this.queue('completeAction', func);
}

BaseUIManager.prototype.queueOnSfdcPageReadyAndCompleteAction = function (func) {
    this.comboQueue(['sfdcPageReady', 'completeAction'], func);
}

BaseUIManager.prototype._superInit = function(){
...
BaseUIManager.prototype._assertPropExists = function(propertyChain){
    var props = dojo.string.splitEscaped(propertyChain, '.');
    var nextProp = this;
    for (var i=0; i < props.length; i++){
        nextProp = nextProp[props[i]];
        if (typeof(nextProp) == 'undefined'){
            alert('Illegal access on ' + propertyChain + ' of BaseUIManager - this property has not been instantiated');
            break;
        }
    }
}

BaseUIManager.prototype._getProp = function(propertyChain, assertExists){
    if (assertExists){
...
BaseUIManager.prototype.getMainLayoutWidget = function(){
    return this.getRootLayoutWidget();
}

BaseUIManager.prototype.layout = function () {
...
BaseUIManager.prototype.$scrollable = function () {
    return $('#formInnerScroll').first();
}

BaseUIManager.scrollToDefaults = function () {
    return {
        duration: 500,
        offset: 0,
...
// TODO: refactor this file to use this method more:
BaseUIManager.prototype.scrollTo = function () {
    var args = [],
        $scrollable = this.$scrollable();
    args.push(arguments[0]);
    if (typeof arguments[2] === "object" && arguments[2]) {
        args.push(arguments[1]);
        args.push($.extend({}, BaseUIManager.scrollToDefaults(), arguments[2]));
    } else if (typeof arguments[1] === "object" && arguments[1]) {
        args.push($.extend({}, BaseUIManager.scrollToDefaults(), arguments[1]));
    } else {
        args.push($.extend({}, BaseUIManager.scrollToDefaults()));
    }
...
BaseUIManager.prototype.validate = function (successFunction, targetDialogKey) {
    ...
    if (typeof targetDialogKey == "string") {
        if (!this.getDialogProps(targetDialogKey, false)) {
            $.error('BaseUIManager.validate could not find the specified dialog: [' + targetDialogKey + ']');
        }
...
BaseUIManager.prototype.validateActiveDialog = function (successFunction, dynamicContentTokenFilter) {
    var args = Array.prototype.slice.call(arguments),
        opts = {
            uiManager: this,
...
BaseUIManager.driverInitDefaults = function () {
    return {
        eventType: 'change',
        animate: true,
        fadeSpeed: 500,
        // rerenderDialogFx: "slide",
        scroll: $.extend({}, BaseUIManager.scrollToDefaults(), {offset: 0}),
        highlight: {
...
BaseUIManager.prototype.initDriver = function (driver, dependents, displayLogic, options) {
    ...
    // this.queueOnSfdcPageReady((function () {
        var uiManager = this,
            ...
            driverOptions = $.extend({}, BaseUIManager.driverInitDefaults(), options, {displayLogic: displayLogic}),
        ...
        */
        ...
        if (parentDialogKey) {
            ...
            scroll = $.extend({}, BaseUIManager.driverInitDefaults().scroll, 
            ...
            highlight = $.extend({}, BaseUIManager.driverInitDefaults().highlight, options ? options.highlight : null);
 ...
 * }
 */
BaseUIManager.prototype.initMultiModeLookup = function (options) {
    var uiManager = this,
...
BaseUIManager.prototype.getRootLayoutWidget = function(assertExists){
    return this._getProp('rootLayout', assertExists);
}

BaseUIManager.prototype.getScrollWidget = function(assertExists){
    return this._getProp('scrollWidget', assertExists);
}

BaseUIManager.prototype.getStatusMessageWidget = function(assertExists){
    return this._getProp('widgets.statusMessage.widget', assertExists);
}

BaseUIManager.prototype.getSubmitBlockingPaneWidget = function(assertExists){
    return this._getProp('widgets.submitBlockingPane.widget', assertExists);
}

BaseUIManager.prototype.getTimedStatusMessageWidget = function(assertExists){
    return this._getProp('widgets.timedStatusMessage.widget', assertExists);
}

BaseUIManager.prototype.getDialogControlsWidget = function(assertExists){
    return this._getProp('widgets.dialogControls.widget', assertExists);
}

BaseUIManager.prototype.getDialogProps = function(dialogKey, assertExists){
    return this._getProp('widgets.dialogs.' + dialogKey, assertExists);
}

BaseUIManager.prototype.getDialogWidget = function(dialogKey, assertExists){
    return this._getProp('widgets.dialogs.' + dialogKey + '.widget', assertExists);
}

BaseUIManager.prototype.getModalDialogController = function(assertExists){
    return this._getProp('modalDialogController', assertExists);
}

/* Modal dialog methods: */

BaseUIManager.prototype.getActiveDialog = function(){
    return this.getModalDialogController(true).getActiveDialog();
}

BaseUIManager.prototype.getActiveDialogManager = function(){
    return this.getModalDialogController(true).getActiveDialog();
}

BaseUIManager.prototype.isDialogActive = function (dialogManagerInstance) {
    return this.getActiveDialogManager() === dialogManagerInstance;
}

BaseUIManager.prototype.getDialogManagerByKey = function(dialogKey){
...
BaseUIManager.prototype.setDialogActionArgs = function () {
    this._dialogActionArgs = Array.prototype.slice.call(arguments);
}

//TODO: refactor:
BaseUIManager.prototype.openDialog = function(/*String*/ dialogKey, /*{action:"", message:"", shortMessage:""}*/ loadActionOpts){
...
BaseUIManager.prototype.reRenderDialog = function(){
    var dialog;
...
BaseUIManager.prototype.reCenterDialog = function(){
    var dialog;
    if (dialog = this.getActiveDialogManager()) {
        dialog.reCenter();
    }
...
BaseUIManager.prototype.reLayoutDialog = function(recalcContentSize){
    var dialog;
    if (dialog = this.getActiveDialogManager()) {
...
//     String shortMessage
// }
BaseUIManager.prototype.submitDialog = function(opts) {
...
BaseUIManager.prototype.closeDialog = function(){
...
BaseUIManager.prototype.closeAllDialogs = function(){
    this.getModalDialogController(true).closeAll();
}

BaseUIManager.prototype.blockDialog = function(mode){
    if (this.getActiveDialogManager()) {
        this.getActiveDialogManager().block(mode);
    }
}

BaseUIManager.prototype.unblockDialog = function(){
    if (this.getActiveDialogManager()) {
        this.getActiveDialogManager().unblock();
    }
}

BaseUIManager.prototype.putDialogContentTokens = function() {
    this._dialogContentTokens = Array.prototype.slice.call(arguments);
}

BaseUIManager.prototype.fireDialogContentsLoaded = function(){
    this.getActiveDialogManager().fireContentsLoaded();
    // this._consumePendingInnerScrollRefresh(false);
    this.layout();
}

BaseUIManager.prototype.fireDialogSubmitFinished = function(){
...
BaseUIManager.prototype.setDialogFocusFieldDomId = function(dialogKey, domId){
...
BaseUIManager.prototype.signalDialogSubmitSuccess = function(dialogKey){
    // If the modalDialogController hasn't been instantiated yet, then the page hasn't finished loading. this will happen during
    // initial load of the (garbage) dialog contents. Since we don't care about these contents, this call can be safely ingnored:
    if (!this.getModalDialogController()) { return; }
...
BaseUIManager.prototype.signalDialogSubmitFailure = function(dialogKey){
    // If the modalDialogController hasn't been instantiated yet, then the page hasn't finished loading. this will happen during
    // initial load of the (garbage) dialog contents. Since we don't care about these contents, this call can be safely ingnored:
    if (!this.getModalDialogController()) { return; }
...
BaseUIManager.prototype.signalFreshDialogContentsReady = function(dialogKey){
    if (!this.getModalDialogController()) { return; }
    this.getDialogManagerByKey(dialogKey).signalFreshDialogContentsReady();
}

BaseUIManager.prototype.setDialogTitleDomId = function(dialogKey, dialogTitleDomId){
    if (!this.getModalDialogController()) { return; }
    this.getDialogManagerByKey(dialogKey).setTitleDomId(dialogTitleDomId);
}

BaseUIManager.prototype.processKeyDown = function(e){
    var keynum;
    var keychar;
    if(window.event){ // IE
        keynum = e.keyCode;
...
BaseUIManager.scrollToJQuery = function () {
    return $("#formInnerScroll .uiManager_scrollToMax, #formInnerScroll .uiManager_scrollTo");
}

BaseUIManager.prototype.onStartAction = function(/*String*/ loadingMessage){
    ...
    this._prevScrollToElements = BaseUIManager.scrollToJQuery();
    */
}

BaseUIManager.prototype.onCompleteAction = function() {
    var modelDialogCntrl, activeDialog;
    ...
    // TODO: should probably remove the following code?:
    /*
    //if (BaseUIManager.fieldErrorJQuery().size() == 0) {
        ...
        var scrollToElements = BaseUIManager.scrollToJQuery();
...
BaseUIManager.prototype.showTimedMessage = function(/*String*/ message, /*int*/ fadeDelay, /*int*/ fadeDuration){
...
BaseUIManager.prototype.innerScrollJQuery = function () {
    return $("#formInnerScroll");
}

BaseUIManager.prototype.initialize = function(){
...
BaseUIManager.prototype._jqueryElementSetup = function () {
    this.killBubbleKeyEvents();
    this.setupRequiredPicklists();
}

BaseUIManager.prototype.killBubbleKeyEvents = function () {
...
BaseUIManager.killBubbleKeyEvents = function (context) {
...
BaseUIManager.prototype.setupRequiredPicklists = function () {
...
BaseUIManager.setupRequiredPicklists = function (context) {
...
BaseUIManager.prototype.notifyInnerScrollContentsRefreshed = function (hasValidationError) {
...
BaseUIManager.prototype._consumePendingInnerScrollRefreshForInit = function () {
...
BaseUIManager.prototype._consumePendingInnerScrollRefresh = function (bypassLayout) {
...
BaseUIManager.fieldErrorJQuery = function() {
...
BaseUIManager.prototype.adjustInnerScrollForError = function () {
    //var target = $('#formInnerScroll div.requiredInput .error');
    var target = BaseUIManager.fieldErrorJQuery();
    if (target.size() > 0) {
...
BaseUIManager.prototype.recalcInnerScroll = function () {
...
BaseUIManager.prototype.relocateSFDCDatePicker = function(){
    VisualforceUtils.moveSalesforceDatePicker(this.getScrollWidget().domNode);
}

BaseUIManager.prototype.destroy = function(){
    BaseUIManager.oldWindowOnload = null;
...
BaseUIManager.prototype.onAfterDestroy = function(){
    // For now, do nothing...
}

BaseUIManager.prototype.onInitializeActionBeforeStart = function(){
    // For now, do nothing...
}

BaseUIManager.prototype.onInitializeActionAfterComplete = function(){
    // For now, do nothing...
}

BaseUIManager.prototype.setPageWidgets = function(widgets){
    this.widgets = widgets;
    this._superInit();
}

BaseUIManager.newWidgetWithTemplate = function(parentWidget, widgetType, widgetProps){
...
BaseUIManager.newWidgetWithoutTemplate = function(parentWidget, widgetType, widgetProps, createInBody){
    var node = document.createElement('div');
    //if (createInBody){
...
// A dojo widget that will inject itself into the root of a Visualforce Page for managing outer shell widget components
// (e.g. dialogs, blocking pages, status messages, etc)

dojo.widget.defineWidget(
   "rkon.widget.UIManagerShell",
   dojo.widget.HtmlWidget, {
   isContainer: true,
   templateString: '<div dojoAttachPoint="containerNode" style="width:100%;height:100%;overflow:hidden;" />',
   
   postCreate: function() {
       var _ui = BaseUIManager.instance;
       ...
       _ui.rootLayout = BaseUIManager.newWidgetWithoutTemplate(this, "RootSmartLayoutContainer", {mixins:{widgetId:"mainWindow"}, styleText:"visibility:hidden;"});
       ...
       if (_ui.widgets.statusMessage) {
           ...
           widgetProps.widget = BaseUIManager.newWidgetWithTemplate(_ui.rootLayout, 'VForceStatusBox', widgetProps);
       }
       if (_ui.widgets.submitBlockingPane) {
           var widgetProps = _ui.widgets.submitBlockingPane;
           widgetProps.widget = BaseUIManager.newWidgetWithoutTemplate(_ui.rootLayout, 'IframeBackingWidget', widgetProps);
       }
       if (_ui.widgets.timedStatusMessage) {
           var widgetProps = _ui.widgets.timedStatusMessage;
           widgetProps.widget = BaseUIManager.newWidgetWithTemplate(_ui.rootLayout, 'TimedStatusBox', widgetProps);
       }
       if (_ui.widgets.dialogs) {
           ...
           dialogControlsProps.widget = BaseUIManager.newWidgetWithTemplate(_ui.rootLayout, 'DialogControls', dialogControlsProps);
           ...
           for (var dialogKey in _ui.widgets.dialogs) {
               ...
               dialogProps.widget = BaseUIManager.newWidgetWithoutTemplate(_ui.rootLayout, 'VForceDialog', dialogProps);
           }
       }
       // create the scroll container:
       _ui.scrollWidget = BaseUIManager.newWidgetWithoutTemplate(_ui.rootLayout, "FixIEScrollContainer", { mixins: {
